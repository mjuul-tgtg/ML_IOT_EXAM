...
def summarize(text, per):
                ...
                if (word.tag_ in ['NN', 'NNS', 'NNP', 'NNPS']):
                    if word.text.lower() not in ['people', 'Joe', 'Rogan']:
                        if word.text not in word_frequencies.keys():
                            word_frequencies[word.text] = 7
                        else:
                            word_frequencies[word.text] += 7
                elif (word.pos_ == "VERB" and word.dep_ in ["ROOT", "advcl"]):
                    if word.text not in word_frequencies.keys():
                        word_frequencies[word.text] = 5
                    else:
                        word_frequencies[word.text] += 5
    for ent in doc.ents:
        for word in ent:
            if (ent.label_ == "PERSON") and (word.pos_ == "PROPN"):
                if ent.text not in word_frequencies.keys():
                    word_frequencies[ent.text] = 13
            elif (ent.label_ == "ORG") and (word.pos_ == "PROPN"):
                if ent.text not in word_frequencies.keys():
                    word_frequencies[ent.text] = 7
    ...
    tags = sorted(word_frequencies.items(), key=lambda kv: kv[1], reverse=True)[:10]

    return tags, summary


def save_to_db_time_slot(transcript_time_slot_id, tags, summary):
    try:
        conn = pymysql.connect(host='127.0.0.1', unix_socket='/tmp/mysql.sock', user='root', passwd=mysql_code,
                               db='mysql')
        cur = conn.cursor()
        mysqlQuery = "INSERT INTO `joe_rogan`.`transcript_time_split_analysis` (`transcript_time_split_id`, `tags`, `spacy_summary`,`spacy_fine_tuned` ) " + \
                     "VALUES ('%s', '%s', '%s', 1);"
...
if __name__ == "__main__":
    ...
    for x in range(146, 269):
        ...
        for i in data:
            ...
            logging.info("done with " + str(tempID))

    for x in range(146, 269):
        logging.info("Starting on " + str(x))
        data = get_data_minute_slot(x)
        for i in data:
            tempEpisodeID = i[1]
            tempID = i[0]
            tags, summary = summarize(i[2], 0.02)
            ...
            for tag in tags:
                tags_cleaned = tags_cleaned + tag[0] + "'',''"
            tags_cleaned = tags_cleaned[:-3]
            save_to_db_time_slot(tempID, tags_cleaned, summary)
            logging.info("done with " + str(tempID))
